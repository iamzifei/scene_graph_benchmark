...
def draw_bb(im, all_rect, all_label, text_file, probs=None, color=None,
            draw_label=True):
    '''
    function borrowed from quickdetection.
    ...
    for i in range(len(all_label)):
        rect = all_rect[i]
        label = all_label[i]
        area = abs(int(rect[2]) - int(rect[0])) * \
            abs(int(rect[3]) - int(rect[1]))
        prob = probs[i] if probs is not None else 0
        f.write(label + " " + str(area) + " " + str(prob) + "\n")
...
def draw_rel(im, all_rel_subj_center, all_rel_obj_center, all_rel_label, probs=None, color=None,
    ...
    for i in range(len(all_rel_label)):
        ...
        def gen_candidate():
            # above of top left
            rel_center = [(subj_center[0] + obj_center[0]) / 2,
